...
<section class="content">
    <div class="row">
        <div class="col-xs-12"> 
            ...
            @endif
            <div class="box">
                <form action="{{ url('/input/perjanjianKinerja2/proses') }}" method="POST">
                    @csrf
                    ...
                    <table id="example1" class="table table-bordered table-striped">
                        <thead style="background-color: #428bca;" id="thead">
                            <tr>
                                ...
                                <th style="color: #ffffff; text-align: center;">Dokumen</th>
                                <th style="color: #ffffff; text-align: center;" id="action">Action</th>
                            </tr>
                        </thead>
                        <tbody id="tabeldata">
                            @foreach ($perjanjianKinerja2s as $key => $perjanjianKinerja)
                            <tr>
                                <td style="text-align: center;">{{ $key + 1 }}</td>
                                <td>{{ $perjanjianKinerja->tahun }}</td>
                                <td>{{ $perjanjianKinerja->data_opd->nama }}</td>
                                <td style="text-align: center;"><a href="{{ asset('file/' . $perjanjianKinerja->file) }}" class="btn btn-success"><i class="fa fa-download"></i></a></td>
                                <td style="text-align: center;">
                                    <div class="col-xs-6" style="padding-right: 5px; padding-left: 0;">
                                        <form
                                            onsubmit="return confirm('Delete this data permanently?')"
                                            action="{{ route('perjanjianKinerja2.destroy', ['id' => $perjanjianKinerja->id ]) }}"
                                            method="POST">
                                            @csrf
                                            <input
                                                type="hidden"
